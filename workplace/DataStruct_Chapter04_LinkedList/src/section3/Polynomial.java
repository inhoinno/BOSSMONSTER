...
public class Polynomial {
	...
	public void addTerm(int coef, int expo){
		...
		// p해당 항 q그 전 항 
		//addTerm메서드는 iterator 를 써보아도 완전하지가 않다는걸 알 수있다.
		*/
		if(coef == 0 ) //exception
			return; //계수가 0인 항
...
////			Term t = terms.get(i); //-> 비효율. get을 새로 계속 호출하니 traversal이 1, 1+1 , 1+1+1 .. 중복되어서 순회됨 
////			result += t.calc(x); //->n개의 노드를 순차방문 하려면 n(n+1)/2 번 돔.
//		}
		while (iter.hasNext()){
			Term t =iter.next();
			result += t.calc(x);
